...
contract LoserPredictionPool is Initializable, PausableUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // share CRPPerBlock
        uint256 lastRewardBlock; // Last block number that CRPs distribution occurs.
        uint256 accRewardTokenPerShare; // Accumulated CRPs per share, times 1e12. See below.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && pool.amount != 0) {
            ...
            accRewardTokenPerShare = accRewardTokenPerShare.add(
                reward.mul(1e30).div(pool.amount)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        totalRewardDebt = totalRewardDebt.add(reward);
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accRewardTokenPerShare).div(1e30).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeCRPTransfer(msg.sender, pending);
